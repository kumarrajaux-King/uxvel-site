...
const LIGHT = {
  '--c-bg':'#FFFFFF','--c-bg2':'#F3FAFA','--c-bg3':'#FFFFFF','--c-card':'#FFFFFF',
  ...
  '--c-accent':'#00A0A1','--c-accent-hi':'#008789','--c-on-accent':'#FFFFFF',
  ...
  '--c-strip':'#E6F4F5','--c-problem':'linear-gradient(180deg,#FFFFFF 0%,#F3FAFA 100%)',
  '--c-services':'linear-gradient(180deg,#FFFFFF 0%,#F3FAFA 100%)','--c-process':'#E9F8F8','--c-work':'#F7F9FB',
  '--c-menu':'#FFFFFF','--c-video':'0',
  '--c-hero':'radial-gradient(110% 85% at 76% 16%,#D0F1F1 0%,rgba(208,241,241,0) 62%),linear-gradient(180deg,#FFFFFF 0%,#F3FAFA 100%)',
  '--c-inv':'#0D3B66','--c-inv-card':'#134472','--c-inv-h':'#FFFFFF','--c-inv-body':'rgba(255,255,255,.70)','--c-inv-body2':'rgba(255,255,255,.78)',
  '--c-inv-ac':'#7FEAEA','--c-inv-ac2':'#6FD3D4','--c-inv-ac2b':'#A3E3E4','--c-inv-ac3':'#14D4D4','--c-inv-deep':'#FFFFFF',
...
const NAVY = {
  ...
  '--c-accent':'#7FEAEA','--c-accent-hi':'#A3E3E4','--c-on-accent':'#082744',
  ...
  '--c-inv-ac':'#00A0A1','--c-inv-ac2':'#33C6C7','--c-inv-ac2b':'#6FD3D4','--c-inv-ac3':'#00C2C4','--c-inv-deep':'#082744',
...
export function install(root, props){
  ...
  function applyTheme(){
    ...
    document.body.style.background = t === 'light' ? '#FFFFFF' : (t === 'navy' ? '#082744' : '#0E1413');
  ...
  function setStep(i){
    ...
    e.nodes.forEach((n,j) => {
      ...
      n.style.background = on ? 'var(--c-accent,#6BD5C6)' : 'var(--c-process,#17211F)';
      n.style.borderColor = on ? 'var(--c-accent,#6BD5C6)' : 'var(--c-line,rgba(255,255,255,.16))';
      ...
      if(lbl) lbl.style.color = cur ? 'var(--c-accent,#6BD5C6)' : (on ? 'var(--c-body2,rgba(255,255,255,.56))' : 'var(--c-muted,rgba(255,255,255,.38))');
    });
    e.rows.forEach((r,j) => {
  ...
  function buildCurtain(){
    ...
    c.style.cssText = 'position:fixed;inset:0;z-index:300;background:' + (S.light ? '#F3FAFA' : (S.navy ? '#082744' : '#17211F')) + ';display:flex;align-items:center;justify-content:center;transition:transform 1.1s cubic-bezier(.76,0,.24,1);will-change:transform';
    ...
    bar.style.cssText = 'position:absolute;left:0;bottom:0;height:2px;width:0%;background:var(--c-accent,#6BD5C6);transition:width 1s cubic-bezier(.3,.8,.2,1)';
